...
#+++ derivative of relu
derivative(f::typeof(relu), x::Float64) = Int(x >= 0)
#+++ derivative of leakyrelu
derivative(f::typeof(leakyrelu), x::Float64) = Int(x .>= 0) + 0.2*Int(x.<0)
...
function backprop!(l::Dense, δnext, zin)
    ...
    #+++
    #+++
    l.δ .= l.W'*(δnext.*derivative.(l.σ,(l.W*zin + l.b)))
    l.∂W .= δnext.*derivative.(l.σ,l.x)*(zin)'
    l.∇b .= δnext.*derivative.(l.σ,l.x)      # (l.W*zin + l.b) = l.x
    return l.δ
end
...
function update!(At::ADAMTrainer)
    ...
    for i in eachindex(At.params)
        ...
        # Take the ADAM step
        #+++
        At.ms[i], At.mhs[i], At.vs[i], At.vhs[i] = m, mh, v, vh
        At.params[i] .= p
    end
...
### Define data, in range [-4,4]
xs = [rand(1).*8 .- 4 for i = 1:2000]
ys = [fsol(xi).+ 0.1.*randn(1) for xi in xs]  ## THIS ONE IS CHANGED FROM TASK3
# Test data
testxs = [rand(1).*8 .- 4 for i = 1:1000]
testys = [fsol(xi) for xi in testxs]
...
### This is the function we want to approximate
fsol(x) = [min(0.5,sin(0.5*norm(x)^2))] # CHANGED THIS FOR TASK 6

### Define data, in range [-4,4]
xs = cat([rand(1).*8 .- 4 for i = 1:2000], [rand(1).*8 .- 4 for i = 1:2000],dims =2)
ys = [fsol(xs[i,:]).+ 0.1.*randn(1) for i=1:2000]  ## THIS ONE IS CHANGED FROM TASK3
# Test data
testxs = cat([rand(1).*8 .- 4 for i = 1:1000], [rand(1).*8 .- 4 for i = 1:1000],dims = 2)
testys = [fsol(testxs[i,:]) for i in 1:1000]
...
# TESTING DELTA COMPUTED CORRECTLY:
l = Dense(1, 3, sigmoid) # 1 output, 3 inputs
z1 = [1.0, 2.0, 3.0]
out1 = copy(l(z1)) # Vector with 1 number
# Derivative of output with respect to itself
δnext = [1.0]
# Calculate gradients
δ = backprop!(l, δnext, z1)
# Try with a slightly different input in second input
z2 = [1.0, 2.0 + 0.0001, 3.0]
out2 = copy(l(z2))
(out2-out1)./0.0001 # Should be roughly the same as δ[2]
δ[2]

# TEST STORE GRAD:
l = Dense(2, 3, sigmoid) # 2 output, 3 inputs
z1 = [1.0, 2.0, 3.0]
out1 = copy(l(z1))
# Derivative of second output with respect to itself
δnext = [0.0, 1.0]
backprop!(l, δnext, z1)
δW = copy(l.∂W) # The first row should be zero, it doesn't affect second output
# Make a small change to l.W
l.W[2,3] += 0.0001
...
# TEST : Define a network with 1 input and 1 output
...
n(x) # Now it is hopefully be slightly closer to 1
